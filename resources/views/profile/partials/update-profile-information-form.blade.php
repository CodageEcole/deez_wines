<section>
    <header>
        ...
        <p>{{ __("messages.profile_information_text") }}</p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="loginForm">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p>{{ __('Your email address is unverified.') }}<button form="send-verification">{{ __('Click here to re-send the verification email.') }}</button></p>
